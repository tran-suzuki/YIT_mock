...
const DezuraTable: React.FC = () => {
  ...
  // Sort workers by company then name for grouping
    ...
    <div className="bg-white rounded-xl shadow-sm border border-slate-200 flex flex-col h-full overflow-hidden">
      ...
      <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-white">
        ...
        <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
           ...
           <span>{tableMode === 'day' ? jpDateString : `${year}年${month}月`}</span>
            ...
            <div className="min-w-[1000px]">
              ...
              <div className="divide-y divide-slate-50">
                {sortedWorkers.map((worker, index) => {
                  ...
                  const isNewCompany = index === 0 || sortedWorkers[index - 1].company !== worker.company;
                  ...
                  return (
                    <div key={worker.id} className={`flex hover:bg-slate-50/50 transition-colors group ${isNewCompany && index !== 0 ? 'border-t-4 border-slate-100' : ''}`}>
                      {/* Worker Info */}
                      <div className="w-60 flex-shrink-0 py-2 border-r border-slate-100">
                      ...
                      <div className="flex-1 relative h-14">
                        ...
                        {record ? (
                          <div 
                            className={`absolute top-1/2 transform -translate-y-1/2 h-8 rounded-md shadow-sm flex items-center justify-center overflow-hidden text-white transition-all hover:scale-[1.02] hover:shadow-md cursor-pointer ${!record.checkOutTime ? 'bg-amber-400 ring-2 ring-amber-100' : 'bg-indigo-500'}`}
                          ...
                          <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 pointer-events-none">
                             <div className="h-1 w-full border-t border-dashed border-slate-300"></div>
                          </div>
                        )}
                      </div>
                    </div>
                   ...
                   return (
                     <div key={worker.id} className={`flex hover:bg-slate-50/50 transition-colors ${isNewCompany && index !== 0 ? 'border-t-4 border-slate-100' : ''}`}>
                        ...
                        {monthDays.map((d) => {
                           ...
                           return (
                             <div key={d} className={`flex-1 min-w-[32px] border-r border-slate-50 flex items-center justify-center h-14 ${isSunday ? 'bg-rose-50/10' : ''}`}>
                                {record ? (
                                  <div className={`w-3 h-3 rounded-full ${record.checkOutTime ? 'bg-indigo-500' : 'bg-amber-400 animate-pulse'}`} title={`${record.checkInTime.substring(11,16)} - ${record.checkOutTime?.substring(11,16) || '作業中'}`}></div>
                                ) : (
                                  <div className="w-1 h-1 rounded-full bg-slate-200 opacity-0 group-hover:opacity-30"></div>
                                )}
                             </div>
                           );
                        })}
